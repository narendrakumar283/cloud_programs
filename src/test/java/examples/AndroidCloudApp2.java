...
public class AndroidCloudApp2
{
	...
	{
		//Desired capabilities related to real android device(as per availaility) in cloud
		String APP="https://github.com/appium/android-apidemos/releases/download/v3.1.0/ApiDemos-debug.apk";
	    DesiredCapabilities caps=DesiredCapabilities.android();
	    caps.setCapability("username", "XXXXXX");
	    caps.setCapability("accessKey", "XXXXXXXXXXX");
		...
		while(2>1)
		{
			...
			catch(Exception ex)
			{
				...
				ta.press(PointOption.point(p)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
				                                                  .moveTo(PointOption.point(0,-1000)).perform();
			}
		}
